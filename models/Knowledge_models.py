...
class KnowledgeMLP_v1(nn.Module):
    def __init__(self, input_size, output_size, kg_size, concat_size):
        super().__init__()
        print(f"input_size: {input_size} | output_size: {output_size} | kg_size: {kg_size} | concat_size: {concat_size}")
        ...
        self.layer2 = torch.nn.Sequential(
            nn.Linear(concat_size, output_size)
        )
        self.kg_layer = torch.nn.Sequential(
            nn.Linear(kg_size, concat_size),
            nn.ReLU()
        )
...
class KnowledgeMLP_v2(nn.Module):
    def __init__(self, input_size, output_size, kg_size, concat_size):
        ...
        self.kg_layer = torch.nn.Sequential(
            nn.Linear(kg_size * 5, concat_size),
            nn.ReLU()
        )
