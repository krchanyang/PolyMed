...
def main(args):
    ...
    polymed = PolyMed(args.data_dir, args.stat_dir, args.train_data_type, args.display, args.plotting, args.integrity)
    ...
    dataset = PolymedDataset(polymed=polymed, train_data_type=args.train_data_type, test_data_type=None, model_name=args.model_name)
    ...
    if args.model_name.lower() == "mlp":
        training_runner = MLPTrainingRunner(train_x, train_y, test_x, test_y, word_idx_case, args, device)
    if args.model_name.lower() == "res":
        training_runner = MLPResNetTrainingRunner(train_x, train_y, test_x, test_y, word_idx_case, args, device)
    if args.model_name.lower() == "graphv1":
        training_runner = GraphV1TrainingRunner(train_x, train_y, test_x, test_y, word_idx_case, org_kb_data, word_idx_total, idx_word_total, word_idx_kb, graph, args, device)
    if args.model_name.lower() == "graphv2":
        training_runner = GraphV2TrainingRunner(train_x, train_y, test_x, test_y, word_idx_case, org_kb_data, word_idx_total, idx_word_total, word_idx_kb, word_idx_allkb, graph, args, device)
        
    training_runner.train()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process some data and generate statistics.')
    parser.add_argument('--data_dir', type=str, default='./data',
                        help='Path to the directory containing the data to be processed. Default is ./data.')
    parser.add_argument('--stat_dir', type=str, default='./data_stat',
                        help='Path to the directory where the generated statistics will be saved. Default is ./data_stat.')
    parser.add_argument('--data_type', type=str, default='extend',
    ...
    parser.add_argument('--integrity', type=bool, default=False,
    ...
    parser.add_argument("--save_base_path", type=str, default="./experiments", help='Base path of the model to save. Default is "./experiments"')
    parser.add_argument("--model_name", type=str, required=True, help="Model Name to train. Supports ml, ML, mlp, MLP, res, Res, graphv1, GraphV1, graphv2, and GraphV2.")
    parser.add_argument("--train_data_type", type=str, help="Train data type. Supports norm, extend, and kb_extend.")
    parser.add_argument("--device", type=int, default=0, help="Specify GPU number. Default is 0.")
    parser.add_argument("--seed", type=int, default=42, help="Set random state. Default is 42.")
