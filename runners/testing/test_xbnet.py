    ...
    def test_xbnet(self):
        ...
        if self.train_data_type == "norm":
            model_saved_path = os.path.join(
                NORM_XBNET_MODEL_SAVE_PATH.format(self.augmentation_strategy),
                XBNET_SAVED_MODEL_NAME,
            )
        ...
        if self.train_data_type == "extend":
            model_saved_path = os.path.join(
                EXTEND_XBNET_MODEL_SAVE_PATH.format(self.augmentation_strategy),
                XBNET_SAVED_MODEL_NAME,
            )
            csv_save_path = os.path.join(
                EXTEND_XBNET_MODEL_SAVE_PATH.format(self.augmentation_strategy),
        ...
        if self.train_data_type == "kb_extend":
            model_saved_path = os.path.join(
                KB_EXTEND_XBNET_MODEL_SAVE_PATH.format(self.augmentation_strategy),
                XBNET_SAVED_MODEL_NAME,
            )
            csv_save_path = os.path.join(
                KB_EXTEND_XBNET_MODEL_SAVE_PATH.format(self.augmentation_strategy),
