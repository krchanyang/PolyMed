...
class Data_variable():
    ...
    def gen_case_data_stat(self, org_data, display, plotting):
        ...
        for d_type in org_data.keys():
            ...
            for ea_data in org_data[d_type]:
                ...
                for norm_comp in ['symptoms', 'diagnosis']:
                    # unique by data
                    temp_by_type[norm_comp] = self.push_data(temp_by_type[norm_comp], ea_data[norm_comp])

                    # unique by all
                    uniq_all_data[norm_comp] = self.push_data(uniq_all_data[norm_comp], ea_data[norm_comp])

                    # accum by data
                    temp_accum_by_type[norm_comp] = self.push_data(temp_accum_by_type[norm_comp], ea_data[norm_comp], p_type='duple')

                    # accum by all
                    accum_all_data[norm_comp] = self.push_data(accum_all_data[norm_comp], ea_data[norm_comp], p_type='duple')
                ...
                if self.data_type == 'extend':
                    for extend_comp in ['sex', 'age', 'family_history', 'background', 'underlying_disease']:
                        # unique by data
                        temp_by_type[extend_comp] = self.push_data(temp_by_type[extend_comp], ea_data[extend_comp])

                        # unique by all
                        uniq_all_data[extend_comp] = self.push_data(uniq_all_data[extend_comp], ea_data[extend_comp])

                        # accum by data
                        temp_accum_by_type[extend_comp] = self.push_data(temp_accum_by_type[extend_comp], ea_data[extend_comp], p_type='duple')

                        # accum by all
                        accum_all_data[extend_comp] = self.push_data(accum_all_data[extend_comp], ea_data[extend_comp], p_type='duple')

                        # count by data
                        temp_count_by_type[extend_comp] = self.push_data(temp_count_by_type[extend_comp], ea_data[extend_comp], p_type='len')

                        # count by all
                        count_all_data[extend_comp] = self.push_data(count_all_data[extend_comp], ea_data[extend_comp], p_type='len')
            ...
            for temp_k in uniq_by_data[d_type].keys():
                info_by_data[d_type]['num_' + temp_k] = len(uniq_by_data[d_type][temp_k])
                info_by_data[d_type]['mean_' + temp_k] = np.mean(count_by_data[d_type][temp_k])
                ...
                info_by_data[d_type]['missing_' + temp_k] = Counter(accum_by_data[d_type][temp_k])[None] / len(count_by_data[d_type][temp_k]) if None in Counter(
                    accum_by_data[d_type][temp_k]).keys() else 0
        ...
        for temp_k in uniq_all_data.keys():
            ...
            info_all_data['missing_' + temp_k] = Counter(accum_all_data[temp_k])[None] / len(count_all_data[temp_k]) if None in Counter(accum_all_data[temp_k]).keys() else 0
    ...
    def gen_knowledge_stat(self, org_data, display, plotting):
        ...
        for d_name in org_data.keys():
            uniq_kb_data['diagnosis'] = self.push_data(uniq_kb_data['diagnosis'], d_name)
            accum_kb_data['diagnosis'] = self.push_data(accum_kb_data['diagnosis'], d_name, p_type='duple')
            count_kb_data['diagnosis'] = self.push_data(count_kb_data['diagnosis'], d_name, p_type='len')

            for norm_comp in ['symptoms', 'relations', 'department', 'location', 'category']:
                uniq_kb_data[norm_comp] = self.push_data(uniq_kb_data[norm_comp], org_data[d_name][norm_comp])
                accum_kb_data[norm_comp] = self.push_data(accum_kb_data[norm_comp], org_data[d_name][norm_comp], p_type='duple')
                count_kb_data[norm_comp] = self.push_data(count_kb_data[norm_comp], org_data[d_name][norm_comp], p_type='len')
        ...
        for temp_k in uniq_kb_data.keys():
            ...
            info_kb_data['missing_' + temp_k] = Counter(accum_kb_data[temp_k])[None] / len(count_kb_data[temp_k]) if None in Counter(accum_kb_data[temp_k]).keys() else 0
    ...
    def data_plot(self, accum_data, knowledge, data_name):
        if not knowledge:
            for key_name, title in zip(['symptoms', 'diagnosis', 'age', 'sex'], ['Symptoms', 'Disease', 'Age', 'Sex']):
                Data_statistic.data_plotting(Counter(accum_data[key_name]).most_common(), title, self.plot_dir, data_name)
        else:
            for key_name, title in zip(['symptoms', 'diagnosis', 'relations', 'department', 'location', 'category'], ['Symptoms', 'Disease', 'Relations', 'Department', 'Location', 'Category']):
                Data_statistic.data_plotting(Counter(accum_data[key_name]).most_common(), title, self.plot_dir, data_name)
...
class Data_statistic():
    ...
    def data_summary(data_info, data_name, table_filp=False):
        if not table_filp:
            ...
            print(format_string.format("name".center(max_name_length), max_name_length, *column_names))
            print("-" * (len(format_string) + 20))
            # Print the data row
            print(format_string.format(data_name.center(max_name_length), max_name_length, *["{:.1f}".format(data_info[name]) if name in data_info else "" for name in column_names]))
            print("=" * (len(format_string) + 20))
        else:
            ...
            print(title_sting.format(f"{data_name}".center(max_name_length), max_name_length*2-6))
            print("=" * (max_name_length * 2 + 7))
            print(format_string.format("State name".center(max_name_length), max_name_length, "result".center(max_name_length), max_name_length))
            print("-" * (max_name_length * 2 + 7))
            # Print the data rows
            for row_name in row_names:
                print(format_string.format(row_name.center(max_name_length), max_name_length, "{:.1f}".format(data_info[row_name]).center(max_name_length), max_name_length))
            print("=" * (max_name_length * 2 + 7))
    ...
    def data_plotting(data, data_name, plot_dir=None, data_lname=None):
        ...
        if data_name == 'Age':
            ...
            for i in range(10):
                ...
                re_y_values.append(np.sum(y_values[np.where((x_values >= (i*10)+1) & (x_values <= (i*10)+10))[0]]))
        ...
        sns.histplot(x=x_values, weights=y_values, palette=cmap, hue=y_values, hue_norm=norm, element="step", fill=True, kde=False, linewidth=0.5, ax=ax,  legend=False)
        ...
        ax.set_title(f"{data_name} Frequency Distribution", fontsize=20, fontweight='bold')
        if len(x_values) > 20:
            ax.set(xticklabels=[])
        plt.tight_layout()
...
class Data_integrity():
    ...
    def completeness_test(data):
        try:
            for record in data:
                if record.get('id') is None or record.get('diagnosis') is None or record.get('symptoms') is None:
                    return False
            return True
    ...
    def consistency_test(data):
        try:
            for record in data:
                if record.get('age') is not None and (record.get('age') < 0 or record.get('age') > 200):
                    return False
    ...
    def duplication_test(data):
        try:
            for record in data:
                if record.get('family_history') is not None and len(record['family_history']) != len(set(record['family_history'])):
                    return False
                if record.get('background') is not None and len(record['background']) != len(set(record['background'])):
                    return False
                if record.get('underlying_disease') is not None and len(record['underlying_disease']) != len(set(record['underlying_disease'])):
                    return False
    ...
    def data_format_test(data):
        try:
            for record in data:
                ...
                if record.get('sex') not in [None, 'M', 'W']:
                    return False
                if record.get('family_history') is not None and not isinstance(record['family_history'], list):
                    return False
                if record.get('background') is not None and not isinstance(record['background'], list):
                    return False
                if record.get('underlying_disease') is not None and not isinstance(record['underlying_disease'], list):
                    return False
    ...
    def kb_duplication_test(data):
        for key, value in data.items():
            for field in ['symptoms', 'relations', 'department', 'location', 'category']:
                if field in value:
...
class Data_preprocessing():
    ...
    def to_numpy_onehot(data, word_idx_book, extend='extend', kb_train=False):
        ...
        multi_on = False
        if extend == 'extend':
            temp_x = np.empty((0, len(word_idx_book['symptoms']) + 2 + 1 + len(word_idx_book['background']) + len(word_idx_book['underlying_disease']) + len(word_idx_book['family_history'])))
        else:
        ...
        for ea_data in data:
            ...
            if extend == 'extend':
                ...
                age_x = np.array([0]) if ea_data['age'] == None else np.array([int(ea_data['age'])])
                ...
                underlying_disease_x = np.zeros(len(word_idx_book['underlying_disease']))
                if ea_data['underlying_disease'] != None:
                    for ea_underlying_disease in ea_data['underlying_disease']:
                        underlying_disease_x[word_idx_book['underlying_disease'][ea_underlying_disease]] = 1
                ...
                if ea_data['family_history'] != None:
                    for ea_family_history in ea_data['family_history']:
                        family_history_x[word_idx_book['family_history'][ea_family_history]] = 1

                empty_x = np.hstack([empty_x, sex_x, age_x, background_x, underlying_disease_x, family_history_x])
            ...
            if multi_on:
                ...
                temp_y = np.vstack([temp_y, np.array([word_idx_book['diagnosis'][ea_data['diagnosis'][0]]])])
...
class Training_data():
    ...
    def __init__(self, polymed, t_type):
        if t_type == 'train':
    ...
    def __load_train_data(self, polymed):
        ...
        self.train_x, self.train_y = Data_preprocessing.to_numpy_onehot(polymed.org_case_data['train'],
                                                                        polymed.data_variable,
                                                                        polymed.data_type,
                                                                        False)
        kx, ky = Data_preprocessing.to_numpy_kb_onehot(polymed.org_kb_data, polymed.data_variable.word_idx_total, extend='extend' if polymed.data_type == 'extend' else False)
        ktrain_x, ktrain_y = Data_preprocessing.to_numpy_onehot(polymed.org_case_data['train'],
                                                                        polymed.data_variable,
                                                                        polymed.data_type,
                                                                        True)

        self.kb_train_x, self.kb_train_y = np.vstack([ktrain_x, kx]), np.hstack([ktrain_y, ky])
    ...
    def __load_test_data(self, polymed):
        ...
        self.single_test_x, self.single_test_y = Data_preprocessing.to_numpy_onehot(polymed.org_case_data['test_single'],
                                                                                    polymed.data_variable,
                                                                                    polymed.data_type,
                                                                                    False)
        self.unseen_test_x, self.unseen_test_y = Data_preprocessing.to_numpy_onehot(polymed.org_case_data['test_unseen'],
                                                                                    polymed.data_variable,
                                                                                    polymed.data_type,
                                                                                    False)
        self.multi_test_x, self.multi_test_y = Data_preprocessing.to_numpy_onehot(polymed.org_case_data['test_multi'],
                                                                                  polymed.data_variable,
                                                                                  polymed.data_type,
                                                                                  False)

        self.kb_single_test_x, self.kb_single_test_y = Data_preprocessing.to_numpy_onehot(polymed.org_case_data['test_single'],
                                                                                    polymed.data_variable,
                                                                                    polymed.data_type,
                                                                                    True)
        self.kb_unseen_test_x, self.kb_unseen_test_y = Data_preprocessing.to_numpy_onehot(polymed.org_case_data['test_unseen'],
                                                                                    polymed.data_variable,
                                                                                    polymed.data_type,
                                                                                    True)
        self.kb_multi_test_x, self.kb_multi_test_y = Data_preprocessing.to_numpy_onehot(polymed.org_case_data['test_multi'],
                                                                                        polymed.data_variable,
                                                                                        polymed.data_type,
                                                                                        True)
    ...
    def __load_graph_data(self, polymed):
        ...
        src, dst = Data_preprocessing.get_kb_relation(polymed.org_kb_data,
                                                      polymed.data_variable.word_idx_allkb)
